...
bool Memory::ReadRaw(unsigned long address, void *buffer, size_t size) {
    ...
    cmdData.command = 1;
    cmdData.data.pid = GetLastProcessId();
    cmdData.data.address = address;
    cmdData.data.buffer = buffer;
    cmdData.data.size = size;

    return Netlink::SendCommand(cmdData) > 0;
}
...
std::string Memory::ReadString(unsigned long address) {
    ...
    int success = ReadRaw(address, &buffer, stringSize);
